...
<nav class="cu-topbar">
    ...
    <div class="cu-topbar-right">
        ...
        <a href="{{ route('customer.payables') }}" class="cu-nav-link active"><i class="fas fa-credit-card"></i><span>My Payables</span></a>
...
<div class="cu-hero">
    <div class="cu-hero-badge">My Payables</div>
    <h1 class="cu-hero-title">Your <span>Outstanding Balance</span></h1>
    <p class="cu-hero-subtitle">Service requests with remaining balance to settle</p>
    @if($payables->count() > 0)
        <div class="cu-stats">
            <div class="cu-stat-box">
                <div class="cu-stat-value">{{ $payables->count() }}</div>
                <div class="cu-stat-label">With Balance</div>
            </div>
            <div class="cu-stat-box">
                <div class="cu-stat-value">₱{{ number_format($payables->sum(function($p) {
                    $verified = $p->payments->where('status', 'verified')->sum('amount');
                    return $p->total_amount - $verified;
                }), 2) }}</div>
                <div class="cu-stat-label">Total Remaining</div>
            </div>
            <div class="cu-stat-box">
                <div class="cu-stat-value">₱{{ number_format($payables->sum(function($p) {
                    return $p->payments->where('status', 'verified')->sum('amount');
                }), 2) }}</div>
                <div class="cu-stat-label">Verified Payments</div>
            </div>
        </div>
    @endif
</div>
...
@if($payables->isEmpty())
    <div class="cu-content">
        <div class="cu-empty-state">
            ...
            <p class="cu-empty-text">No outstanding balances. All payments are up to date.</p>
                ...
                <tbody>
                    @foreach($payables as $sr)
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="cu-vehicle-plate">{{ $sr->vehicle_name }} / {{ $sr->vehicle_model }}</div>
                                <div class="cu-vehicle-details">{{ $sr->vehicle_registration }}</div>
                            </td>
                            <td>
                            ...
                            <td class="cu-amount-green">₱{{ number_format($sr->total_amount, 2) }}</td>
                            <td class="cu-amount-green">₱{{ number_format($verifiedAmt, 2) }}</td>
                            <td>
                            ...
                            <td>
                                @if($balance > 0 && !$hasPending)
                                    {{-- Has balance, no pending submission --}}
                                    <button 
                                        type="button" 
                                        class="cu-button-pay"
