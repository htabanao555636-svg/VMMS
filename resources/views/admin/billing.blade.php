...
@section('content')
    ...
    <style>
        ...
        .act-verify {
            ...
            color: white;
            box-shadow: 0 3px 8px rgba(45, 155, 111, 0.35);
        }

        .act-verify:hover {
            box-shadow: 0 5px 14px rgba(45, 155, 111, 0.45);
        }
        ...
        <div class="billing-tabs">
            <button class="billing-tab-btn" id="tab-btn-downpayment" onclick="switchTab('downpayment', this)">
                ...
                @if($downpayments->total() > 0)
                    <span
                        style="background:#e5e7eb;color:#374151;padding:1px 7px;border-radius:20px;font-size:10px;margin-left:4px;">{{ $downpayments->total() }}</span>
                @endif
            </button>
            <button class="billing-tab-btn" id="tab-btn-balance" onclick="switchTab('balance', this)">
                ...
                @if($balancePayments->count() > 0)
                    <span
                        style="background:#e5e7eb;color:#374151;padding:1px 7px;border-radius:20px;font-size:10px;margin-left:4px;">{{ $balancePayments->count() }}</span>
                @endif
            </button>
            <button class="billing-tab-btn" id="tab-btn-fullypaid" onclick="switchTab('fullypaid', this)">
                ...
                @if($fullyPaid->total() > 0)
                    <span
                        style="background:#e5e7eb;color:#374151;padding:1px 7px;border-radius:20px;font-size:10px;margin-left:4px;">{{ $fullyPaid->total() }}</span>
                @endif
            </button>
        </div>
                ...
                <div class="table-scroll-wrapper">
                    <table class="tab-table">
                        ...
                        <tbody>
                            @foreach($downpayments as $sr)
                                ...
                                <tr>
                                    <td>
                                        <div style="font-weight:700;" class="copy-id"
                                            onclick="copyToClipboard('#{{ str_pad($sr->id, 4, '0', STR_PAD_LEFT) }}', this)">
                                            #{{ str_pad($sr->id, 4, '0', STR_PAD_LEFT) }}</div>
                                    ...
                                    <td>
                                        @if($payment?->proof_image)
                                            <button class="act-btn act-view"
                                                onclick="viewProof('{{ $payment->proof_image }}')"
                                                title="View Proof">
                    ...
                    <div class="pagination-wrapper">
                        <div class="pagination-summary">
                            Showing
                            <strong>{{ $downpayments->firstItem() }}</strong>–<strong>{{ $downpayments->lastItem() }}</strong>
                                ...
                                <ul class="pagination-list">
                                    ...
                                    @else
                                        <li class="page-item"><a class="page-btn"
                                                href="{{ $downpayments->previousPageUrl() }}&tab=downpayment">&#8592; Prev</a></li>
                                    @endif
                                    @foreach(range(1, $downpayments->lastPage()) as $page)
                                        ...
                                        @elseif(abs($page - $downpayments->currentPage()) <= 2 || $page == 1 || $page == $downpayments->lastPage())
                                            <li class="page-item"><a class="page-btn"
                                                    href="{{ $downpayments->url($page) }}&tab=downpayment">{{ $page }}</a></li>
                                    ...
                                    @if($downpayments->hasMorePages())
                                        <li class="page-item"><a class="page-btn"
                                                href="{{ $downpayments->nextPageUrl() }}&tab=downpayment">Next &#8594;</a></li>
                                    @else
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($sr->latest_pending_payment?->proof_image)
                                            <button class="act-btn act-view"
                                                onclick="viewProof('{{ $sr->latest_pending_payment->proof_image }}')"
                                                title="View Proof">
                                                <i class="fas fa-eye"></i>
                                            </button>
                                        @else
                                    ...
                                    <td>
                                        <div class="actions-row">
                                            @if($sr->latest_pending_payment)
                                                <form action="{{ route($prefix . '.billing.verify', $sr->latest_pending_payment) }}"
                                                    method="POST"
                                                    onsubmit="event.preventDefault(); showConfirm('Verify Payment', 'Are you sure you want to verify this payment?', this)">
                                                    @csrf
                                                    <button type="submit" class="act-btn act-verify" title="Verify Payment">
                                                ...
                                                <form action="{{ route($prefix . '.billing.reject', $sr->latest_pending_payment) }}"
                                                    method="POST"
                                                    onsubmit="event.preventDefault(); showConfirm('Reject Payment', 'Are you sure you want to reject this payment?', this)">
                                                    @csrf
                                                    <button type="submit" class="act-btn act-reject" title="Reject Payment">
                        ...
                        <tbody>
                            @foreach($fullyPaid as $sr)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route($prefix . '.service-request.show', $sr) }}" class="act-btn act-view"
                                            title="View Request">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                    </td>
                        ...
                        @if($fullyPaid->hasPages())
                            <nav class="pagination-nav">
                                <ul class="pagination-list">
                                    ...
                                    @else
                                        <li class="page-item"><a class="page-btn"
                                                href="{{ $fullyPaid->previousPageUrl() }}&tab=fullypaid">&#8592; Prev</a></li>
                                    @endif
                                    @foreach(range(1, $fullyPaid->lastPage()) as $page)
                                        ...
                                        @elseif(abs($page - $fullyPaid->currentPage()) <= 2 || $page == 1 || $page == $fullyPaid->lastPage())
                                            <li class="page-item"><a class="page-btn"
                                                    href="{{ $fullyPaid->url($page) }}&tab=fullypaid">{{ $page }}</a></li>
                                    ...
                                    @if($fullyPaid->hasMorePages())
                                        <li class="page-item"><a class="page-btn"
                                                href="{{ $fullyPaid->nextPageUrl() }}&tab=fullypaid">Next &#8594;</a></li>
                                    @else
                                        <li class="page-item disabled"><span class="page-btn">Next &#8594;</span></li>
                                    @endif
                                </ul>
